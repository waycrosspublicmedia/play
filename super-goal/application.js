System.register([], function (_export, _context) {
  ...
  function createApplication(_ref) {
	...
	function registerLoader() {
		var binLoader = cc.assetManager.downloader['_downloaders']['.bin'];

      function downloadBin(url, options, onComplete){

		binLoader(url.replace("@","__0AT0__"),options,onComplete); 
      }
		var CconbLoader = cc.assetManager.downloader['_downloaders']['.cconb'];
      function downloadCconb(url, options, onComplete){

		CconbLoader(url.replace("@","__0AT0__"),options,onComplete);
      }
        var pngLoader = cc.assetManager.downloader['_downloaders']['.png'];

        function downloadPng(url, options, onComplete) {

            pngLoader(url.replace("@", "__0AT0__"), options, onComplete);
        }
        var jsonLoader = cc.assetManager.downloader['_downloaders']['.json'];

        function downloadJson(url, options, onComplete) {

            jsonLoader(url.replace("@", "__0AT0__"), options, onComplete);
        }
